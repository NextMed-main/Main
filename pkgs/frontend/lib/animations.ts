 ...
 */
export const scaleFadeIn = (config?: AnimationConfig): string => {
  const cfg = mergeConfig(config);
...
export const animationClasses = {
  ...
  fadeOut: (duration: number = 300) =>
    `animate-out fade-out duration-${duration}`,
   ...
   * ズームイン
   */
  zoomIn: (duration: number = 300) =>
    `animate-in zoom-in-95 duration-${duration}`,

  /**
   * ズームアウト
   */
  zoomOut: (duration: number = 300) =>
    `animate-out zoom-out-95 duration-${duration}`,
