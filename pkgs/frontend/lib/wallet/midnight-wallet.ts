...
export async function connectLace(
...
): Promise<import("@midnight-ntwrk/dapp-connector-api").DAppConnectorWalletAPI> {
  if (!isLaceInstalled()) {
    throw new Error(
  ...
  try {
    ...
    if (typeof mnLace.connect === 'function') {
      ...
      const api = await (mnLace.connect as (networkId: string) => Promise<import("@midnight-ntwrk/dapp-connector-api").DAppConnectorWalletAPI>)(networkId);
      return api;
    }
    ...
    if (typeof mnLace.enable === 'function') {
      ...
      const api = await (mnLace.enable as () => Promise<import("@midnight-ntwrk/dapp-connector-api").DAppConnectorWalletAPI>)();
      return api;
    }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("rejected") || error.message.includes("cancelled") || error.message.includes("Rejected")) {
      ...
      if (error.message.includes("Unsupported network")) {
        throw new Error(`Network mismatch. Please ensure your Lace wallet is configured for the '${networkId}' network.`);
      }
      throw error;
    }
