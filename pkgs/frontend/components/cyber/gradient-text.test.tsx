...
describe("GradientText", () => {
	...
	describe("基本レンダリング", () => {
		...
		it("デフォルトでspanタグとしてレンダリングされる", () => {
			const { container } = render(
				<GradientText>テキスト</GradientText>
			);
		...
		it("カスタムクラス名を適用できる", () => {
			render(
				<GradientText className="custom-class">テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
	...
	describe("グラデーションバリアント", () => {
		it("defaultグラデーションを適用する", () => {
			render(
				<GradientText gradient="default">テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
		...
		it("secondaryグラデーションを適用する", () => {
			render(
				<GradientText gradient="secondary">テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
		...
		it("accentグラデーションを適用する", () => {
			render(
				<GradientText gradient="accent">テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
		...
		it("rainbowグラデーションを適用する", () => {
			render(
				<GradientText gradient="rainbow">テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
			expect(element.className).toContain("from-indigo-500");
		...
		it("sunsetグラデーションを適用する", () => {
			render(
				<GradientText gradient="sunset">テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
	...
	describe("アニメーション", () => {
		it("animate=trueでアニメーションクラスを適用する", async () => {
			render(
				<GradientText animate={true}>テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
			// useEffectが実行されるまで待つ
			await waitFor(() => {
		...
		it("animate=falseでアニメーションクラスを適用しない", () => {
			render(
				<GradientText animate={false}>テキスト</GradientText>
			);
			const element = screen.getByText("テキスト");
