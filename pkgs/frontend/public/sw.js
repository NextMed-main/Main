...
self.addEventListener("fetch", (event) => {
  ...
  if (isStaticAsset(url)) {
    ...
    event.respondWith(
      cacheFirst(request, STATIC_CACHE, 30 * 24 * 60 * 60 * 1000, 100),
    );
  } else if (isImage(url)) {
    // CacheFirst strategy for images (maxEntries: 60)
    event.respondWith(
      cacheFirst(request, IMAGE_CACHE, 30 * 24 * 60 * 60 * 1000, 60),
    );
  } else if (isFont(url)) {
    // CacheFirst strategy for fonts
    event.respondWith(
      cacheFirst(request, FONT_CACHE, 365 * 24 * 60 * 60 * 1000, 20),
    );
  } else if (isAPIRequest(url)) {
    // NetworkFirst strategy for API requests
    event.respondWith(
      networkFirst(request, API_CACHE, 10000, 5 * 60 * 1000, 50),
    );
  } else {
    // Default: NetworkFirst for navigation and other requests
    event.respondWith(
      networkFirst(request, STATIC_CACHE, 10000, 5 * 60 * 1000, 100),
    );
  }
});
